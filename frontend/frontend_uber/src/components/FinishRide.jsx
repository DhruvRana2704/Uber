...
const FinishRide = (props) => {
    ...
    async function endRide() {
        console.log('Ride Data',props.rideData._id)
        if (!props.rideData || !props.rideData.user) {
            console.error('Ride data or user data is missing');
            return;
        }
        try {
    ...
    return (
        <div>
            <h5 onClick={() => { props.setFinishRidePanel(false) }} className='p-1 text-center absolute top-0 w-[93%]'><i className="text-3xl text-gray-200 ri-arrow-down-wide-line"></i></h5>
            ...
            <div className='flex gap-2 flex-col justify-between items-center'>
                ...
                <div className='w-full mt-5'>
                    ...
                    <div className='flex items-center gap-5 p-3 border-b-2'>
                        ...
                        <div><h3 className='text-lg font-medium'>2012/8</h3><p className='text-sm -mt-1 text-gray-600'>{props.rideData.pickup}</p></div>
                    </div>

                    <div className='flex items-center gap-5 p-3 border-b-2'>
                        <i className="text-lg ri-map-pin-user-fill"></i>
                        <div><h3 className='text-lg font-medium'>2012/8</h3><p className='text-sm -mt-1 text-gray-600'>{props.rideData.destination}</p></div>
                    </div>

                    <div className='flex items-center gap-5 p-3'>
                        <i className="text-lg ri-money-rupee-circle-line"></i>
                        <div><h3 className='text-lg font-medium'>₹{props.rideData.fare}</h3><p className='text-sm -mt-1 text-gray-600'>Cash Cash</p></div>
                    </div>

                </div>

                <div className='mt-6 w-full'>
                    <button onClick={endRide} className='w-full mt-5 text-lg flex items-center justify-center bg-green-600 text-white font-semibold p-3 rounded-lg'>Finish Ride</button>
                    <p className='text-red-700 mt-10 text-xs flex justify-center'>click on finish ride if you have got the payment</p>
                </div>
            </div>
        </div>
    )
